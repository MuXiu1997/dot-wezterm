...
local truncate_string = ____helpers.truncate_string
...
local tmux_variables = {
    ...
    H = function() return wezterm.hostname() or "localhost" end,
    I = function(ctx)
        local ____opt_2 = ctx.tab
        return tostring(____opt_2 and ____opt_2.tab_index or 0)
    end,
    W = function(ctx)
        if not ctx.tab then
            return "zsh"
...
local special_variables = {
    NORD_TMUX_STATUS_DATE_FORMAT = function(_ctx) return wezterm.strftime("%Y-%m-%d") end,
    NORD_TMUX_STATUS_TIME_FORMAT = function(_ctx) return wezterm.strftime("%H:%M:%S") end,
