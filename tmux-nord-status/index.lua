...
function ____exports.apply_to_config(config)
    ...
    wezterm.on(
        ...
        function(window, _pane)
            window:set_left_status(wezterm.format(left_status_format_expr:eval({window = window})))
            window:set_right_status(wezterm.format(right_status_format_expr:eval({window = window})))
        end
    )
    wezterm.on(
        "format-tab-title",
